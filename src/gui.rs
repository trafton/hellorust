use rltk::{RGB, Rltk, Console, Point, VirtualKeyCode, BTerm};
use specs::prelude::*;
use crate::{CombatStats, Equipped, InBackpack, Map, Name, Player, Position, RunState, State, Viewshed};
...
pub fn draw_ui(ecs: &World, ctx: &mut Rltk) {
    ...
    //gamelog
    let logs = ecs.fetch::<GameLog>();

    let mut y = 44;
    for log in logs.entries.iter().rev() {
        if y < 49 { ctx.print(2, y, log);}
        y += 1;
    }
